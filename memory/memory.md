...
- The web frontend is deployed at `https://sharpfights.vercel.app` with the Vercel project Root Directory set to `apps/web`. Liveblocks and Upstash production connectivity are verified; authenticated two-browser Friend/Instant acceptance is still required before calling online PvP production-ready.
...
- Current verification for this replacement: 91 unit tests across 20 files passed; all workspace typechecks and lint passed; the Next.js 16.3.1 production webpack build passed and emitted every new API route; all 15 development gameplay browser journeys and all 3 production PWA journeys passed; `git diff --check` passed. Production HTTP, Redis, and Liveblocks connectivity checks pass. Provider-backed authenticated two-browser and staged load gates remain outstanding.
...
- The serverless multiplayer implementation is complete and the repository is linked to the `sharpfights` Vercel project. Production and the single ignored local `apps/web/.env` contain the required app identity, Liveblocks, and Upstash configuration while preserving the existing Clerk variables.
...
- Provider secrets are server-only: never add a `NEXT_PUBLIC_` prefix, never paste them into documentation or logs, and never commit any local `.env` file. Vercel environment changes apply only to new deployments, so redeploy after provisioning. Then run real Friend and Instant matches with two separate authenticated accounts before describing online PvP as production-ready.

### Production provider provisioning checkpoint

- Provisioned the Vercel Marketplace Upstash Redis resource `upstash-kv-lime-mountain` and connected it to the `sharpfights` project. Production has the integration-managed `KV_*` variables plus sensitive `UPSTASH_REDIS_REST_*` aliases used by the app.
- Added the stable app identity secret and Liveblocks server/webhook secrets to Vercel Production. Local development now uses one ignored `apps/web/.env`; the root `.env` and redundant `apps/web/.env.local` were removed. Secret values were never staged or printed.
- Commit `22f8e80` deployed successfully to production. Verification returned HTTP 200 for the homepage and `/api/connectivity`, the expected 404 for a missing invite, 400 for an unsigned webhook, and 401 for unauthenticated friend creation. Direct provider checks returned Upstash `PONG` and Liveblocks HTTP 200.
- This proves configuration and service reachability, not the complete multiplayer journey. Before calling online PvP production-ready, still run Friend and Instant matches between two separate authenticated accounts, then the missed-event, reconnect/takeover, queue-storm, and staged load gates.
